...
func (priv *PrivateKey) derive() (pub *PublicKey) {
	...
	Q := priv.secp256k1.ScalarBaseMult(priv.D)

	/* Check that Q is on the curve */
	if !priv.secp256k1.IsOnCurve(Q) {
...
func GenerateKey(secp256k1 EllipticCurve, rand io.Reader) (priv PrivateKey, err error) {
	/* See Certicom's SEC1 3.2.1, pg.23 */
...
func (priv *PrivateKey) FromBytes(b []byte, curve EllipticCurve) (err error) {
	if len(b) != 32 {
		return fmt.Errorf("Invalid private key bytes length %d, expected 32", len(b))
	}

	priv.D = new(big.Int).SetBytes(b)
	priv.secp256k1 = &curve
	/* Derive public key from private key */
	priv.derive()
...
func (priv *PrivateKey) FromWIF(wif string, curve EllipticCurve) (err error) {
	/* See https://en.bitcoin.it/wiki/Wallet_import_format */
	...
	err = priv.FromBytes(priv_bytes, curve)
	if err != nil {
		return err
	}
...
func (pub *PublicKey) FromBytes(b []byte) (err error) {
	...
	if b[0] == 0x02 || b[0] == 0x03 {
		...
		P, err := pub.secp256k1.Decompress(new(big.Int).SetBytes(b[1:33]), uint(b[0]&0x1))
		if err != nil {
		...
		if !pub.secp256k1.IsOnCurve(pub.Point) {
